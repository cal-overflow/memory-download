  ...
  ipcMain.on("beginDownload", (event, { input, output, options }) => {
    ...
    downloadMemories(input, output, options, sendMessage)
      ...
      .catch((err) => {
        if (isDebugging)
          console.log(
            `An error occurred while downloading memories. Error: ${err.message}`
          );

        window.webContents.send("message", {
          message:
            "An unknown error occurred while processing your memories.<br />Please try again",
          error: err,
        });
      })
      .finally(() => {
