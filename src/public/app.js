...
const downloadLocationButton = document.getElementById(
...
const progressUpdatesOption = document.querySelector(
  'form [name="progress-updates"]'
);
...
let prevConcurrentSetting,
  prevShowProgressUpdatesSetting,
  prevShowPreviewsSetting;
let isReAttemptingFailedMemories = false;
...
ipcRenderer.on("message", (event, data) => {
  ...
  if (data.totalMemories) {
    ...
    if (data.photos || data.videos) {
      feedbackLink.setAttribute(
        "href",
        `${feedbackUrl}&memoryTotal=${data.totalMemories}&photos=${data.photos}&videos=${data.videos}&version=${appVersion}`
      );
    }
  ...
  if (data.count && !isReAttemptingFailedMemories) {
    ...
    if (concurrentOption.checked) {
      if (progressUpdatesOption.checked) {
        document
          .getElementById("advanced-progress-updates")
        ...
        if (!document.getElementById(`year-${data.date.year}`)) {
          ...
          const table = document.querySelector(
            "#advanced-progress-updates tbody"
          );
        ...
        document.querySelector(
          `#year-${data.date.year} .${data.type}`
        ).innerHTML = `${data.count} / ${
          count[data.date.year][`${data.type}Total`]
        }`;
      }
  ...
  if (data.isReAttemptingFailedMemories) {
    ...
    progressBarNote.innerHTML =
      "<em>The progress bar does not update for re-attempted downloads</em><br />";
  ...
  if (data.smallError) {
    document.querySelector("#small-error-feedback pre").innerHTML =
      data.smallError.message;
  ...
  if (data.isComplete) {
    if (total === failedMemories.length) {
      showErrorMessage({
        message:
          "Either there is no internet connection or the input file provided is expired<br />Be sure to use data from your Snapchat account that was downloaded within the last 7 days",
      });
...
const handleStepChange = (i) => {
  if (i == 0 || step == 5) ipcRenderer.send("reload");
    ...
    if (step !== 0)
      document.getElementById(`${step}`).classList.toggle("bg-light");
    if (step !== 0)
      document.getElementById(`${step}`).classList.toggle("text-dark");
...
const showErrorMessage = ({ message, error }) => {
  ...
  if (error) {
    document.querySelector("#extra-error-information pre").innerHTML =
      error.message;
  }
